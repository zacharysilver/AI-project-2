...
data = data[['track_popularity', 'danceability', 'energy', 'key', 'loudness', 'mode', 'speechiness', 'acousticness', 'instrumentalness', 'liveness', 'valence', 'tempo', 'duration_ms']]
        ...
        if column != 'track_popularity': 
            ...
            for i in range(1, len(sorted_data)):
                ...
                if ge_true != 0 and ge_false != 0:
                    gt_entropy = -((ge_true / (ge_true + ge_false)) * np.log2(ge_true / (ge_true + ge_false)) + (ge_false / (ge_true + ge_false)) * np.log2(ge_false / (ge_true + ge_false)))
                entropy = (lt_true + lt_false) / len(sorted_data) * lt_entropy + (ge_true + ge_false) / len(sorted_data) * gt_entropy
