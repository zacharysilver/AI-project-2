...
def kNN(normalized_data):
        ...
        for fold in range(5):
            ...
            for testRowIndex, testRow in test_data.iterrows():
                ...
                tmpTrainData['EucliDistanceToCurrTestDatum'] = tmpTrainData['original_index'].map(
                    lambda idx: distanceMatrix[int(testRow['original_index']), idx])
        ...
        for i, k in enumerate([1, 3, 5]):
            ...
            print(f"k={k}: Mean Accuracy = {mean_acc:.4f}, Standard Deviation = {std_acc:.4f}")
